...
| Node | What it does | Runs on |
|------|--------------|---------|
| `twin_control` | Move a twin through one or more environment waypoints. For an arm moving to a single waypoint, an optional fixed tool offset lets waypoints be authored in a mounted sensor's frame (e.g. a wrist camera) while the arm still commands its own link. | Either |
