...
- **Any sim runtime**: locomotion and flight control surfaces — `twin.commands.*`, `twin.locomotion.*`, `twin.flight.*` and their twin shortcuts (`twin.move_forward()`, `twin.takeoff()`, `twin.land()`, …). In MuJoCo a drone flies under thrust, so it obeys gravity and contacts; see [Drone commands](/feature-reference/drone-commands) for the takeoff-altitude rules and the current limits.
- **Not in simulation**: the `imu` sensor — raises `NotSimulatedError` (live/driver-only).
