...
For YOLO models, extra keyword arguments pass through to Ultralytics — `model.predict(img, imgsz=512, iou=0.45, max_det=10)` — and an unrecognised name is rejected rather than ignored. Other edge runtimes still ignore keyword arguments they don't recognise.
