...
### Velocity- or effort-only targets

For applications using `cw.mqtt.update_joints_state`, set `as_targets=True`
to send commands. You may pass `joint_positions={}` with a non-empty
`velocities` or `efforts` dictionary when the target robot supports that
control mode. Do not fill position targets with zeros merely to send an effort
command: that would also request a position change.

Omitted channels are not commanded; a supplied zero is a real target.
Measured-state updates retain their position requirement. Check the robot's
supported control modes, units, limits and stop behavior before sending
commands. A successfully published message does not prove that the robot
supports or executed it.
...
<Note>stub — pending human curation.</Note>
...
```python
cw.affect("simulation")
twin = cw.twin("unitree/go2")

# Blocking — streams the sim camera until Ctrl+C
twin.start_streaming()

# Non-blocking — starts the simulation in the background
async def start():
    await twin.stream_video_background()
...
video = twin.camera.get_video()            # connects to the live WebRTC stream
video.show()                               # live window; press q/Esc to quit
frame = video.get_frame(format="numpy")    # or read frames yourself
```
...
### Timestamped custom camera sources

{/* stub — this section will be curated by a human before publishing */}

For asynchronous `VirtualCameraStreamer` sources, return a cached
`CapturedVideoFrame` from `get_frame`. It pairs RGB pixels with Unix and
monotonic acquisition times (seconds) and an increasing acquisition ID.
Create it once when a new source state is acquired, not on every stream tick.
Repeated sends keep that timestamp; `None` displays a placeholder without a
capture timestamp. Existing RGB-array callbacks remain supported.

Source timestamps alone do not verify recording/Replay synchronization.
See the [SDK example](https://github.com/cyberwave-os/cyberwave-python#timestamped-virtual-camera-sources).
...
```python
# Import from HuggingFace. Any repository can be imported and its format
# detected, but only robotics datasets (like LeRobot) are correctly shown
# and usable today.
...
# Zip upload only supports LeRobot (v2.1/v3) today — see the import format
# matrix linked below for other formats via HuggingFace.
...
See [Import Datasets](/feature-reference/datasets/import) for the full format support matrix.
