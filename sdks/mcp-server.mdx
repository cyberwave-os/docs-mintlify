...
The MCP server exposes 27 tools across six categories.
...
| `cw_list_assets` | List assets, optionally filtered by workspace |
...
| `cw_create_twin` | `name` (required), `asset_uuid` (required), `environment_uuid?`, `description?`, `metadata?`, `execute=false` |
