...
## Slugs vs. catalog origin

A Controller Policy's `slug` identifies that specific policy instance — its workspace and name. It does not indicate which catalog template, if any, the policy was originally created from; that is tracked separately and is not part of the slug system.
