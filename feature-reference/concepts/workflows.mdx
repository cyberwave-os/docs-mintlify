...
Alerts are raised explicitly: wire a `send_alert` node downstream of `call_model`, optionally gated by `detection_event_gate` (class filtering, `on_enter` / `on_change` semantics) and `timed_condition` (cooldowns / dwell). The legacy implicit `emit_event` config on `call_model` is no longer compiled — see the [`emit_event` migration](/use-cyberwave/workflows#migrating-from-emit_event).
...
See [Edge Workers](/feature-reference/workflows/workers/overview) for the full lifecycle, eject pattern, and generated worker format.
...
Activation validates the workflow first: it must have at least one trigger node and at least one non-trigger (action) node — a trigger with nothing downstream can never do anything — plus every mandatory input on the trigger's chain must be configured. If validation fails, activation is blocked and the error names the specific node and field to fix.
