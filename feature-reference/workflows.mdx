...
Workflows can run **on the cloud** (Celery tasks) or **on the edge device** depending on the trigger type. Cloud triggers handle schedule and manual execution today; webhook and email triggers are implemented but not yet exposed in the node palette (see the trigger table below). The `camera_frame` trigger runs ML inference directly on the edge — no video leaves the device.
...
<CardGroup cols={2}>
  ...
  <Card title="Transform & Routing" icon="shuffle">
    Reshape, convert, route, multiplex, or fan out data: `json_parser`, `annotate`, `anonymize`, `send_depth` (edge-only)
  </Card>
  ...
  <Card title="Integration" icon="plug">
    Talk to external systems and run user-supplied logic: `http_request`, `send_email`, `send_mqtt`, `code` (edge-only)
  </Card>
...
| **Webhook** | Cloud (Celery) | HTTP POST to a per-node webhook URL. *Coming soon — not yet available in the node palette.* |
...
| **Email** | Cloud (Celery) | Incoming email. *Coming soon — not yet available in the node palette.* |
...
For open-vocabulary edge models (YOLOE-26 text/visual, YOLO-World), set the **Prompt** field on the `call_model` node to drive `set_classes` at inference time. The camera worker forwards the static prompt into `model.predict(..., prompt=...)`. See [Prompting Call Model](./workflows/call-model-prompt) for the full guide, including the comma-list vs. single-phrase semantics and the camera-only limitation that reference-wired prompts are not supported.
...
Activating a `run_on_edge` workflow auto-publishes the `sync_workflows` MQTT command for each referenced twin, so the edge picks up the worker within seconds — no manual step. The editor locks the graph while active, so the natural edit flow (Deactivate → edit → Activate) republishes through the same signal. {/* stub */}

Reach for a manual sync only when the activation cycle can't fix it — e.g. a broker outage or edge-core restart swallowed the original nudge.
...
### Running, Stopping, and Deactivating

The header Play button (and the matching row button in an environment's workflow list) reflects what it can actually do at that moment:

- **Nothing running** — shows **Run**. Clicking triggers a manual execution.
- **An execution is currently active** — shows **Stop**. Clicking cancels that execution (`cw.workflow_runs.cancel(...)` / the dashboard's Cancel action). For workflows running on an edge device, canceling now actually interrupts the in-flight worker on the device — not just backend bookkeeping. (One narrow exception: a workflow with more than one `camera_frame` trigger on different twins doesn't yet stop mid-invocation on cancel — it still cancels in the backend, just not on-device for that specific shape.)
- **Active on an automated trigger (schedule, MQTT, camera_frame, …) with nothing currently running** — shows **Deactivate**. There's nothing to cancel yet, so the button turns the workflow off instead.

The button never shows a generic "always green" active indicator disconnected from real execution state — what you see is either idle, actually running, or armed-and-waiting, never ambiguous between the two.
