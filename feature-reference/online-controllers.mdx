...
## Running a skill from the Control assistant

In **Simulate → Agent**, describe the skill and any run limit, then select
**Plan**. Check the selected robot, policy and limits before executing; start
the required simulation first. The assistant follows the controller's current
run status, and cancelling that action stops only that run. A completed run
does not prove task success—check its policy evidence and Replay separately.

The assistant keeps a trained skill's saved settings unless it exposes a
supported command input. Use a **policy-step limit** to bound these runs;
a duration alone is not currently supported. A robot accepting velocity
commands does not mean every policy on that robot accepts speed or gait overrides.
