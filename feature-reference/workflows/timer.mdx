...
<Note>
`duration` must be set before the workflow can be activated. Like any mandatory
(`*`) input, a Timer with no duration blocks activation and the editor shows a
"Configure Duration" prompt on the node.
</Note>
