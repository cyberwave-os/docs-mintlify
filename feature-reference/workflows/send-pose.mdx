...
**In Live, the robot must be connected.** A live pose is refused unless the robot's
edge device is publishing health, so a pose is never silently delivered to nobody.
When that happens the run **fails** with the reason, and a warning alert is raised on
the robot telling you which of the three causes it is and what to do:

| What happened | What to do |
|---|---|
| The robot is not paired to an edge device | Pair it from the robot's page |
| The edge device is online but the robot's driver is not running | Start the driver |
| The edge device has gone quiet | Check that it is powered on and on the network |

This is always on — there is no setting to turn it off, because a pose that reaches
nobody is indistinguishable from one that worked. **Simulation is exempt:** poses are
always sent there, whether or not any robot is connected.

## Configuration flow

1. **Target Twin** — pick the twin to control. Only **joint-controllable** twins (robots with movable joints) appear in the list. **Required.**
2. **Twin Poses** — pick a **saved pose** for that twin, or **Custom Pose** to author a new one. **Required.**

<Note>
Both **Target Twin** and **Twin Poses** are mandatory (`*`). A workflow with a
Send Pose that is missing either can't be activated — the editor flags it and
activation returns an error. Positions/velocities/efforts maps refine a pose;
they don't replace picking one.
</Note>
...
If a pose contains a joint the selected robot doesn't have — usually a pose copied
from a different robot — saving is blocked and the joint is named, rather than the
pose being sent with that joint quietly missing.
...
| `pose_name` | string | Yes | Saved (or custom) pose to send. Set by the Twin Poses picker. |
...
| `sent` | boolean | Always `true` — if the pose could not be sent the node fails instead. |
