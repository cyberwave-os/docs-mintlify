...
|---|---|---|
| `image_bytes` | object | Raw image bytes to anonymise. Wire from `data_source.image_bytes` or any upstream node that emits bytes. Required on cloud chains; on edge camera-frame chains the worker passes the upstream frame straight in. |
| `image_url` | image | Optional echo only — used as the value of `source_image_url` in the output. Bytes always come from `image_bytes`. |
...
| Field | Type | Description |
|---|---|---|
| `anonymized_image_bytes` | object | Raw bytes of the anonymised image, re-encoded in the source format (JPEG / PNG). Canonical handoff — wire a downstream node's `image_bytes` to this field via the InputMappingEditor. |
| `anonymized_image_base64` | image | Data URL (`data:image/...;base64,...`) of the anonymised image. UI preview channel — consumed by the workflow run-history thumbnail. Preserves the source MIME type, falling back to PNG when the source format can't be detected — echoing a 100 KB JPEG back as a megabyte PNG data-URL would balloon downstream storage. |
