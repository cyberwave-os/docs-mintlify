...
Driver and worker logs also report bus membership directly. `Zenoh session has
no remote peers` means that container is alone on the bus — the other side is
down, or multicast discovery is blocked on this host and you need to set
`ZENOH_CONNECT` on every participant. `Zenoh session linked to N remote peer(s)`
confirms they found each other. A container that starts before its counterpart
waits a few seconds before warning, so the line means a real problem rather than
normal startup skew.
