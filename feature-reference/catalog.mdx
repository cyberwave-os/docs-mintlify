...
The SDK supports large GLB uploads by automatically switching to an attachment + signed-URL flow
when files exceed the standard upload limit. Create the asset first, then upload the mesh by UUID:
...
Primitive assets are curated public catalog entries with a short **alias** (e.g. `camera`,
`lidar`). They can be instantiated directly by alias without knowing the full `vendor/slug`
registry ID — making it easy to quickly populate an environment.
