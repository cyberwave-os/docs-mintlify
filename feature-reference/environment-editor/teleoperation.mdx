...
<Note>
  For the inverse — moving the arm by hand and having the twin follow it — see
  [Freedrive](/use-cyberwave/environment-editor/freedrive).
</Note>
...
That is intentional. Subclass [`BaseDriver`](/feature-reference/edge/drivers/base-driver-class) (or declare the same commands in `cw-driver.yml`), run the driver, and the keyboard UI follows the manifest. See [Keyboard controller from a driver YAML](/feature-reference/keyboard-controller-from-driver-yaml).
...
- **Radians / degrees** is a per-panel toggle: the overlay and the controller card each convert their own joint readout.
...
poses stay independent: they receive the same *step*, not the same absolute position.
...
### Controllers that only work on real hardware

{/* stub — this section will be curated by a human before publishing */}

Some controllers drive the physical robot's driver directly and do nothing in
simulation:

- **Keyboard (Driver)** — not compatible with simulation. Cyberwave warns you
  when you enter Simulation mode; twins will not react to those keypresses.
  There is no automatic replacement.
- **Local controller** — not valid in simulation. Cyberwave warns you; there is
