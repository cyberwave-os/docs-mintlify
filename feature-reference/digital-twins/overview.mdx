...
| `type` | enum | Sensor type (`rgb`, `depth`, `lidar`, `lidar_2d`, `lidar_3d`, `lidar_4d`, `radar`, `map`, `audio`, `audio_mono`, `audio_stereo`, `gps`, `compass`, `imu`, `flashlight`). |

Declaring a `gps` sensor is what makes a twin eligible to be positioned from its GNSS fixes in Live mode — see [Geo reference](/feature-reference/environment-editor/geo-reference).
...
| `min_range`, `max_range` | number | Optional range values (depth/lidar sensors). `max_range` sets how far a lidar/radar coverage preview reaches. |
| `fov_horizontal`, `fov_vertical` | number | Optional horizontal/vertical FOV values. Used to draw lidar/radar coverage previews (horizontal sweeps up to 360° are supported). |
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    icon="microchip"
    href="#pairing-a-twin-with-physical-hardware"
  >
