...
| **Source files** | The full source tree, including read-only Cyberwave-generated files. |
...
Action terms are authored from five control-mode presets, plus a code-backed `custom` escape hatch:
...
| **Custom (code)** | `custom` | defined by your `ActionTermCfg` class/factory | — (the class defines its own joints) |
...
The **Custom (code)** preset is the exception: it references a user-owned `ActionTermCfg` class or factory by `module` + `symbol` and does **not** render a joint table — the class defines its own joint targeting. See [Custom (code-backed) terms](#custom-code-backed-terms).
...
| `custom` | User code referenced by `module` + `symbol` (function or class). See [Custom (code-backed) terms](#custom-code-backed-terms). |
...
## Custom (code-backed) terms (stub)

Observations, actions, and commands each expose a `custom` term — the escape hatch for task-specific Python that isn't in the shared runtime library. All three are authored the same way:

- **Reference** the code by a dotted **module** + **symbol** (e.g. `smoothed_blocking` + `SmoothedBlockingDeltaPositionActionCfg`). The module maps to a task-local source file (`a.b.c` → `a/b/c.py`).
- **Kind** — observations and actions can be a **function** or a **class**; **commands are always a `CommandTermCfg` subclass** (no Kind selector).
- **Author the file** with **Create & edit file** (opens the Source files editor with a starter scaffold) or by uploading it — there is no inline editing on the term tabs. Use **Open in editor** to edit an existing referenced file, or **Use an existing file…** to point the term at a source file that already exists (fills the module / command name for you).
- **Custom actions do not touch the joints** — the class/factory defines its own joint targeting *and* control math, so no joint table and no `scale` / `offset` / `baseline_delta` fields are shown. Those values come from the class itself or from `kwargs`.
- **Custom observations declare an Output dimension** — the number of floats the function/class returns per step. The runtime can't introspect user code, so this width is required: without it the exported observation layout has no offsets for this term (or any term after it in the group) and RL config generation fails.

The referenced code is uploaded as a task-local source file and wired at runtime through the generated `make_actions()` / `make_observations()` / command managers (`custom_terms` registry). Extra `kwargs` round-trip on save; author them via the SDK.
...
`observation_input` nodes pick a group and a subset of term names from the **Observations** tab. Each term shows read-only tensor metadata — an `image-like` / `flat` badge and a short kind label — so the builder can route image-like terms into a `cnn_backbone` and reject flat terms feeding one. If a selected term is a `custom` term with no declared Output dimension, the builder raises a blocking error and Save is disabled until you set the width in the Observations tab — the runtime can't infer a custom term's size, so this would otherwise only fail at export. NatureCNN and IMPALA are fixed conv stacks; switch a `cnn_backbone` to the **Custom** preset (or use `cnn3d_backbone`) to edit an explicit `layers` stack — per-layer stride, kernel, padding, channels and pooling — with labeled fields for the common torch modules and a raw JSON fallback for the rest. Gaussian log-std controls (`initial_log_std`, `min_log_std`, `max_log_std`) live in the actor head inspector and apply only to the stochastic Gaussian policy.
...
Commands are authored as first-class **terms**, like observation terms: each term references a `CommandTermCfg` subclass by `module` + `symbol`. The command **name** is the mjlab command key; the **module** defaults to `commands.<name>` (`commands/<name>.py`) but is editable — point a term at **any** source file (type the module or use **Use an existing file…**), just like custom observations and actions (e.g. `env_cfg`, `my_commands`). Add and edit terms in the editor's **Commands tab** (term name, module, cfg symbol, lifecycle fields, kwargs) or from the SDK (`set_training_command_spec` / `set_inference_command_spec`). The term's Python is authored in the Source files editor via **Create & edit file** (or uploaded) — there is no inline editor. See [Custom (code-backed) terms](#custom-code-backed-terms).

The Commands tab has two collapsible lifecycle sections — **Training commands** and **Inference commands**. Each starts collapsed with a **Set up** button that both enables the lifecycle and opens its authoring UI (with a first term ready to edit); **Turn off** clears the spec by design (a disabled lifecycle is "deliberately off", not a forgotten upload). One term can drive both managers — author it in one section and tick **Also …** on its card to mirror it into the other. From the SDK, pass `enabled=` to `set_training_command_spec` / `set_inference_command_spec` to set a lifecycle explicitly, or omit it and it is derived from whether the spec has commands.
