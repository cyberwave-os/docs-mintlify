...
<Note>
Set `CYBERWAVE_MQTT_USER` to your Cyberwave username — the `username` field from
`GET /api/v1/users/@me/status/`, not your email address and not a twin UUID. The
broker re-checks authorization periodically using only the username, so a value
that matches no account works until that re-check and then starts failing.
</Note>

```bash
# Switch to remote operation (cloud/teleop)
mosquitto_pub -h mqtt.cyberwave.com -p 8883 \
  -u "$CYBERWAVE_MQTT_USER" -P "$CYBERWAVE_API_KEY" \
