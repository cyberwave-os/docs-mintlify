...
def define_interface(self, iface):
  iface.add_listener(
      ...
      protocol=ProtocolArgs(source_types=[SOURCE_TYPE_TELE]),
  )
  iface.add_publisher(
      TopicSpec(
...
A driver publishes state as `edge` and listens for commands as `tele`, so the two
declarations never share a value. Import them from `cyberwave.constants` rather
than spelling them inline — a string outside the enum is accepted here and
...
| 5 | `twin.commands.rotate(axis="yaw", amount_deg=30, source_type="tele")` |
