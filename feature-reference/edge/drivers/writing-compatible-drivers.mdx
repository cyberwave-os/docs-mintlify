...
| -------- | ----------------------------- |
| **Web UI** | Dynamic control surfaces — **Keyboard (Driver)** lists this twin's `commands.supported` entries, uses only the intersection with saved key bindings, and respects `commands.specs[name].continuous` / `rate_hz`. See [Live Teleoperation](/feature-reference/environment-editor/teleoperation#keyboard-driver). |
