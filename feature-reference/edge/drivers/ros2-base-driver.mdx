...
| **No `PublisherArgs.rate_hz` with `from_ros`** | Use **`TopicSpec.rate_hz`** on the topic spec to cap MQTT outbound |
...
### Arm drivers: mixin listener + `from_ros` publisher

For a full manipulator stack, **do not** register `joint/update` yourself. Compose [driver mixins](/feature-reference/edge/drivers/driver-mixins). The **[Piper driver](https://github.com/cyberwave-os/cyberwave/blob/main/cyberwave-edge-nodes/piper/piper_driver.py)** shows the full integration.

| Registration | Owner |
| ------------ | ----- |
| `joint/update` listener (`tele` / `edit` / `sim_tele`) | `JointControllerMixin` (via `super().define_interface`) |
| `twin/command` arm verbs | `ArmCapabilityMixin` (via `super().define_interface`) |
| `joint/update` publisher (`edge`) | **Your** `define_interface` → `from_ros=Ros2TopicSpec(...)` |

```text
MQTT joint/update (tele)  →  JointControllerMixin  →  submit_joint_targets  →  inbox
  →  tick() / pump_joint_commands  →  _publish_joint_command  →  ros_publish(cmd)

MQTT twin/command         →  ArmCapabilityMixin  →  IK  →  same JointController path

ROS joint_states_single   →  Ros2JointFeedbackMixin  →  _joint_states (IK, gripper latch)
                         →  from_ros forwarder       →  mqtt.publish @ rate_hz (edge)
```

**Critical:** `define_interface` must call `super().define_interface(iface)` **before** adding the `from_ros` publisher so mixin listeners are registered exactly once.

---

## Composing mixins on `BaseROS2Driver`

See **[Driver mixins and capabilities](/feature-reference/edge/drivers/driver-mixins)** for the Piper walkthrough. Minimum integration for a teleoperable ROS arm:
...
class PiperDriver(
    ...
    def tick(self):
        self.pump_joint_commands()
```

| Integration point | Why it matters |
| ----------------- | -------------- |
| MRO: `JointControllerMixin` before `ArmCapabilityMixin` | Arm IK targets and MQTT teleop share `_apply_joint_targets` |
| `_init_joint_feedback()` + `_init_joint_command_buffer()` in `__init__` | Allocates `_joint_states` and the thread-safe command inbox |
| `super().define_interface` then `from_ros` publisher | Mixins own inbound catalog; driver owns outbound edge feedback |
| `tick()` → `pump_joint_commands()` | Moves MQTT-thread targets onto the ROS executor via `_publish_joint_command` |
| `joint_controller_motion() → None` | Passthrough when the vendor shapes motion in ROS (Piper vel/effort on `JointState`) |
...
ROS sensor topics often exceed 100 Hz. The `from_ros` forwarder and any custom ROS callback throttle outbound Cyberwave publishes with a per-topic cap that inherits `BaseDriver`'s resolution (default **30 Hz**):

| API | Purpose |
| --- | ------- |
| `ros_stream_publish_max_hz(ros_topic)` | Resolves the cap: per-publisher `TopicSpec(rate_hz=...)` → manifest `mqtt_max_hz:` → `BaseDriver.STREAM_PUBLISH_MAX_HZ` (default 30 Hz) — no separate ROS-only default |
...
| ------- | --------- | ---- |
| **`from_ros=Ros2TopicSpec(...)`** | ROS → Cyberwave | Telemetry, joints, poses — **preferred for edge drivers** |
| **`CallbackGroup(fn)` + `from_ros`** | ROS → Cyberwave | Transform/filter before MQTT (`convert_joints_to_payload` seam) |
...
| **`JointControllerMixin`** | Cyberwave → ROS (joint teleop) | Registers `joint/update` listener — do not duplicate |
| **`twin.joints.set()`** | Cyberwave → MQTT | **Clients only** — application teleop, not driver feedback |
