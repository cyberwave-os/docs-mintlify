...
| `TELEMETRY_PUBLISH_RATE_HZ` | `1.0` Hz for built-in `twin/telemetry` publisher |
...
| Publisher `twin/telemetry` | `cyberwave/twin/{uuid}/telemetry` | Debounced **`driver_info`** @ `TELEMETRY_PUBLISH_RATE_HZ` (default 1 Hz) |
...
`CommandArgs(name=..., continuous=..., rate_hz=...)` is what **Keyboard (Driver)** reads. Discrete commands (`continuous=False`, the default) fire once per key — use that for `capture_photo`. Continuous commands repeat while the key is held. Joint teleop is separate: held keys stream setpoints; a tap still sends one step. [Keyboard (Driver)](/feature-reference/environment-editor/teleoperation#keyboard-driver) · [from a driver YAML](/feature-reference/keyboard-controller-from-driver-yaml)
...
| API | Purpose |
| --- | ------- |
| `STREAM_PUBLISH_MAX_HZ` | Class attribute, default **30 Hz** — the cap applied to throttled streams |
| `stream_publish_max_hz(stream_key)` | Resolves the effective cap for `stream_key` (see resolution order below) |
...
**Resolution order** for the effective cap (highest wins): a per-publisher `TopicSpec(rate_hz=...)` on the `from_ros`/`add_publisher` entry → the node manifest's top-level `mqtt_max_hz:` value → the `STREAM_PUBLISH_MAX_HZ` class default. No environment variable controls this.
...
## Edge health (twin liveness)

`BaseDriver` automatically starts an `EdgeHealthCheck` publisher on `cyberwave/twin/{uuid}/edge_health` right after the MQTT interface is wired, and stops it on shutdown — no lifecycle wiring required.

| API | Purpose |
| --- | ------- |
| `edge_health_extras()` | Override to merge driver-specific fields into every heartbeat (default `{}`) |
| `edge_health_streams()` | Override to return `[(stream_id, stream_config), ...]` registered on the publisher (default `[]`) |
| `_touch_edge_health()` | Call from a data-received callback to mark the stream alive |

```python
def edge_health_extras(self) -> dict:
    return {"ctrl_mode": self._ctrl_mode}
...
<Warning>
`cyberwave-edge-core` also runs its own `EdgeHealthCheck` per linked twin for host-level metrics (memory, CPU temp, watchdog). A driver container's `edge_health_extras()` is a stopgap reuse of the same publish mechanism for driver-specific fields — a dedicated per-driver telemetry channel is planned to replace it.
</Warning>
...
class MyDriver(BaseDriver):
    ...
    TELEMETRY_PUBLISH_RATE_HZ = 1.0        # built-in twin/telemetry publisher
    STREAM_PUBLISH_MAX_HZ = 30.0           # cap for acquire_stream_publish_slot()
