...
If your recordings include microphone audio, tick **Include audio** when creating the dataset (or set `include_audio: true` on the create request). It is **off by default**, so existing exports are unchanged.

When enabled, the export bundle gains an `audio/` folder:
