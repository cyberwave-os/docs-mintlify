...
Depth Anything V2 and Video-Depth-Anything run at 518px by default. Inference
cost is roughly quadratic in that value, so lowering it is the cheapest speed-up
on CPU-only devices. Set **Depth Input Size** in the `call_model` node's Inputs
section — the row appears only when the selected model is a Depth-Anything one.
It must be a multiple of 14 (518, 392, 322, 266); a bad value is rejected when
the workflow compiles, not on the device.

| `input_size` | Relative cost | Use for |
|---|---|---|
| `518` | 1.0x | Default. Best detail. |
| `392` | ~0.6x | Mild speed-up, little visible loss. |
| `322` | ~0.4x | CPU-only devices. |
| `266` | ~0.25x | Lowest latency; coarse edges. |

Pair this with the ViT-S ("Small") checkpoint and a lower camera-trigger `fps`.
On an NVIDIA or Jetson host the worker uses the GPU automatically — see
...
To opt in, enable the flag on **both** the publisher and any same-host consumer, and give each container a shared IPC namespace (`--ipc=host`) and a raised locked-memory limit (`memlock=-1` — the default 8 MB cap is too small for a frame pool, and without it the transport silently falls back to copy):
