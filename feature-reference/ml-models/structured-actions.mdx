...
Python SDK as `cyberwave.STRUCTURED_ACTIONS`, and served live at
...
for action in STRUCTURED_ACTIONS:
    print(action.id, action.output_format, action.description)
```

The SDK mirror carries `id`, `label`, `description`, `output_format`, and
`requires_image`. Prompt templates and result schemas are server-side — read
them from `GET /api/v1/mlmodels/structured-actions` when you need the live
contract.
...
When you call the Python SDK, `result.output` is already parsed JSON. When you
call the HTTP API, parse as standard JSON and branch on `output_format`:
...
3. Mirror the action in
   `cyberwave-sdks/cyberwave-python/cyberwave/models/playground.py`
   (`STRUCTURED_ACTIONS`). The SDK test
   `tests/test_mlmodels_actions.py::TestBackendAlignment` will fail until
   the two files agree.
