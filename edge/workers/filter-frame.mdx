...
If the worker stops publishing, or publishes a stale frame older than the driver's freshness window (~200 ms), the driver emits a black frame so privacy never silently regresses.
