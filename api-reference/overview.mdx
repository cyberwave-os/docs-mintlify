...
All REST endpoints require an API key. Go to the [Cyberwave dashboard](https://cyberwave.com/profile) → **Profile** → **API Tokens** and create a new key, then pass it in the `Authorization` header on every request.

| Field | Value |
|---|---|
| Base URL | `https://api.cyberwave.com` |
| Auth header | `Authorization: Bearer <CYBERWAVE_API_KEY>` |
| Alternative header | `X-Cyberwave-Api-Key: <CYBERWAVE_API_KEY>` |
| Accepted prefixes | `Bearer` and `Token` |
