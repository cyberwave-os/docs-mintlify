...
Sends a navigation command to the robot. Published by the backend in response to a REST API call. Two dispatch shapes cover single-point and multi-point targets:

```json
// "goto" — single target, list-form position/rotation
{
...
```json
// "path" — ordered waypoints, dict-form position/rotation per waypoint
{
  "action_id": "d0b0e8a7-7d6e-4e73-bf2d-9c420ff14c75",
  "command": "path",
  "twin_uuid": "twin-uuid-here",
...
| `command`                | string     | yes      | `"goto"` \| `"path"` \| `"stop"` \| `"pause"` \| `"resume"` \| `"relative_move"` \| `"takeoff"` \| `"hover"` \| `"land"` |
...
| `position`               | array\[3\] | no       | `goto` target `[x, y, z]` in metres (list form)                   |
| `rotation`               | array\[4\] | no       | `goto` target orientation as quaternion `[x, y, z, w]` (list form) |
| `waypoints`              | array      | no       | `path` target: ordered `{id?, position: {x,y,z}, rotation?: {w,x,y,z}, duration_seconds?, actions?}` entries (dict form) |
| `constraints`            | object     | no       | Navigation constraints (planner-specific)                         |
| `frame_id` / `reference_frame` | string | no    | Frame `position`/`waypoints` are expressed in. Omitted, or a map/environment frame name (e.g. `"map"`) → coordinates are environment-absolute. Set to one of **the twin's own link names** (e.g. `"base_link"`, `"end_effector_link"`) → `position`/`rotation` are an offset from that link instead, and the driver resolves the offset into a joint-space move (IK/motion planning) itself — the platform never sends joint angles on this topic. Only `goto` supports a link-relative target today. |
| `metadata`               | object     | no       | Extra metadata forwarded to the driver                            |
| `timestamp`              | number     | yes      | Unix timestamp (seconds)                                          |

A driver should only ever have **one in-flight goal**: a new `goto`/`path` supersedes whatever mission is currently running, and `stop` must actually preempt it, not just acknowledge it.
...
| `take_photo`                   | UGV Beast, Quadruped | Trigger camera snapshot (result published to [`camera/photo`](#cyberwavetwintwin_uuidcameraphoto)) |
...
A single still image captured on demand. This is the **result** of a [`take_photo`](#cyberwavetwintwin_uuidcommand) command:
...
| Field         | Type   | Required | Description                                        |
| ------------- | ------ | -------- | -------------------------------------------------- |
| `source_type` | string | yes      | Always `"edge"` (see [source types](#source-types)) |
| `image`       | string | yes      | Base64-encoded image bytes (decode to get the file) |
| `format`      | string | yes      | Image encoding, always `"jpeg"`                    |
| `width`       | number | no       | Frame width in pixels                              |
| `height`      | number | no       | Frame height in pixels                             |
| `timestamp`   | number | no       | Unix timestamp (seconds)                           |

#### Retrieving the photo

Subscribe to the topic, then base64-decode the `image` field. Remember to prepend the [topic prefix](#topic-prefix) in non-production deployments (e.g. `localcyberwave/twin/...`).
...
def on_message(client, userdata, msg):
    ...
    if payload.get("source_type") != "edge" or not payload.get("image"):
        return
    with open("photo.jpg", "wb") as f:
        f.write(base64.b64decode(payload["image"]))
    print(f"Saved {payload['width']}x{payload['height']} photo")

client = mqtt.Client()
client.on_message = on_message
client.connect("your-broker-host", 1883)
# Trigger the capture, then wait for the result.
client.subscribe(f"cyberwave/twin/{TWIN_UUID}/camera/photo")
client.publish(
    f"cyberwave/twin/{TWIN_UUID}/command",
    json.dumps({"source_type": "tele", "command": "take_photo", "timestamp": 0}),
)
client.loop_forever()
```
