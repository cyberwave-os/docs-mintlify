...
<Tabs>
  ...
  <Tab title="Python SDK">
    ...
    cw = Cyberwave(api_key="your_api_key", source_type="sim")
    ...
    <Info>
    The `source_type` parameter tells the backend how to route your commands: `"sim"` for simulation, `"tele"` for teleoperation, or `"edit"` for the environment editor. You can also omit it and the SDK will use the default.
    </Info>
  </Tab>
</Tabs>
...
Your application code from Part 1 now drives the physical robot, switch the source type to target real hardware:

```python
cw = Cyberwave(api_key="your_api_key", source_type="tele")
```
