...
<Tip>
  Want to try Cyberwave without any setup first? See [Try a Real Robot
  Live](/get-started/try-real-robot) to control a real SO101 arm from your
  browser in under 2 minutes.
</Tip>
...
<CardGroup cols={3}>
  <Card title="Python SDK" icon="code" href="/sdks/python-sdk">
    Control twins, capture frames, trigger workflows, and manage infrastructure
    programmatically
  </Card>
  <Card title="MCP Server" icon="plug" href="/sdks/mcp-server">
    Let AI agents (Claude, GPT, Cursor) manage your robotics setup through
    natural language
  </Card>
  <Card
    title="REST API"
    icon="brackets-curly"
    href="/api-reference/rest/DefaultApi"
  >
...
<Info>
  If you're only working in simulation or building application code, you can
  skip Part 2 for now and come back when you're ready to connect hardware.
</Info>

#### Step 5: Install the Cyberwave CLI

> Using a Raspberry Pi to connect to your robots? [Check out how to set it up here!](https://docs.cyberwave.com/edge/raspberry-pi)
...
<Info>
  This step is optional. If you skip it, `cyberwave edge install` will prompt
  you to log in interactively during installation. Pre-authenticating is useful
  if you're scripting the setup or want to avoid the interactive login during
  install.
</Info>
...
<Check>
  Your physical robot is now paired with its digital twin and syncing in real
  time.
</Check>
...
<CardGroup cols={3}>
  ...
  <Card title="UGV Beast" icon="robot" href="/hardware/ugv/get-started">
