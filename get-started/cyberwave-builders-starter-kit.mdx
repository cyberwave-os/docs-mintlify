...
description: "A detailed onboarding guide for core builders designing and deploying physical AI agents with Cyberwave."
icon: "toolbox"
---

## 0. How to Use This Starter Kit
...
**One mandatory action for all core builders:** Complete **Section 5 (Designing Your Physical AI Agent)** in this guide and submit your physical agent idea back to us so we can work on it together.

<Warning>
The submission of your physical agent design is mandatory if you are a core builder. Please complete Section 5 and share your idea with us on Discord.
</Warning>
...
A vertical AI agent is essentially an LLM that has been given a domain-specific brain, a memory, specialised skills, and the ability to use external tools. Instead of being a generalist that knows a little about everything, it goes deep into one domain; understanding the terminology, workflows, edge cases, and decision patterns that matter in that specific industry.

**Core components of a Vertical AI Agent:**

```mermaid
flowchart LR
    LLM["LLM Module"] --> MEM["Memory Module"]
    LLM --> CSM["Cognitive Skills Module"]
    LLM --> TM["Tools Module"]
```
...
A defining capability that separates physical AI agents from their software counterparts is the ability to understand and reason about **physical dynamics**, forces like gravity, friction, inertia, material properties, and spatial geometry.

This understanding enables physical agents to:
...
The cognition component is the agent's **brain**. It takes what perception observed, considers the current goal, and decides what to do next. 

**Key principle**: This could be as simple as a rule ("if obstacle ahead, turn left") or as advanced as a VLA model generating motor commands from a camera feed. The key requirement is that every decision must be **physically executable**, if the hardware can't do it, it's a bad decision.
...
**Perceive** (what is happening?) --> **Reason** (what should I do given my goal, context, and memory?) --> **Act** (execute the command on hardware) --> **Perceive** (what happened as a result?) --> **Reason** (did it work? should I adjust?) --> **Act** (next action) --> repeat
...
Self-driving cars operate in complex urban environments with pedestrians, traffic signals, and unpredictable road conditions. They use LiDAR, cameras, radar, and GPS for perception. A fine-tuned LLM evaluates contextual inputs and generates planning strategies. Cognitive models handle path planning, collision avoidance, and motion prediction. Actuators control steering, braking, and acceleration with millisecond-level precision.

**Warehouse Robotics**

Large-scale warehouses require robots that can pick, sort, transport, and replenish products across thousands of bins. These agents combine SLAM (Simultaneous Localisation and Mapping) for navigation, object recognition for product identification, and grippers for physical manipulation. A vertical AI agent manages inventory forecasting and task assignment, while physical agents execute the logistics.

**Surgical Robotics**

Minimally invasive surgery requires precision that exceeds human capability. Surgical robots use high-resolution cameras and force sensors for perception, medical LLMs for surgical planning referencing patient data and guidelines, and micro-precision actuators for executing incisions and sutures with real-time haptic feedback.

**Manufacturing**

Automotive assembly lines automate welding, painting, and quality inspection. Vision models detect defects, manufacturing LLMs coordinate workflows and optimise resource allocation, and robotic arms execute precise operations. Quality inspection runs continuously alongside production.

**Agriculture**

Precision farming uses drones with multi-spectral cameras and soil sensors for crop health monitoring. Agricultural LLMs analyse sensor data against weather forecasts and historical patterns to optimise irrigation, predict yield, and direct autonomous harvesters and sprayers.

<Note>
In every industry, the pattern is the same: **perception feeds cognition, cognition directs actuation, actuation generates feedback, and feedback improves perception**. As the specific sensors, models, and actuators change, the architecture does not.
</Note>
...
- **Edge runtime** = the Cyberwave stack installed on that computer — this is what makes the hardware work with Cyberwave
...
1. **Edge core** — a host-level service that manages identity, security, and device registration
2. **Runtime services** — modular Docker containers that handle specific tasks (MQTT bridge, ROS nodes, sensor drivers, etc.)
...
```mermaid
graph TB
    subgraph CLOUD["☁️ Cyberwave Cloud"]
        direction LR
        DT["Digital Twins"] ~~~ API["API / SDK"] ~~~ ML["ML Models & Workflows"]
    end

    CLOUD <-->|"Cloud Sync"| EDGE

    subgraph EDGE["🖥️ Cyberwave Edge"]
        direction LR
        CORE["Edge Core"] ~~~ DRIVER["Cyberwave Hardware Driver"]
    end
    ...
    subgraph HW["🤖 Robot Hardware"]
        direction LR
        SENSORS["Sensors"] ~~~ ACTUATORS["Actuators"] ~~~ MCU["Microcontroller"]
    end
```
...
### 4.1 The Edge Runs Everywhere

One of Cyberwave's core design principles is that the edge is **device-agnostic**. The same Cyberwave Edge stack runs on:

- Your **laptop** (macOS, Linux, Windows)
- A **Raspberry Pi** (4B/5)
- An **NVIDIA Jetson** (Nano, Orin)
- An **industrial PC or NUC**
- Any **Linux-based compute** with Docker support
...
**Configuration 1: SO101 Robot Arms (Edge on Laptop)**

```mermaid
graph LR
    CYB["Cyberwave Cloud"] <-->|"Cloud Sync"| EDGE["Cyberwave Edge<br/>(Edge Core + SO101 Driver)"]
    EDGE <-->|"USB-C"| LEADER["Leader Arm"]
    EDGE <-->|"USB-C"| FOLLOWER["Follower Arm"]
    EDGE <-->|"USB"| CAM["Camera"]
```
...
**Configuration 2: UGV Beast Rover (Edge on Raspberry Pi)**

```mermaid
graph LR
    CYB["Cyberwave Cloud"] <-->|"Cloud Sync"| EDGE["Cyberwave Edge<br/>(Edge Core + UGV Driver<br/>on Raspberry Pi)"]
    EDGE <-->|"Serial UART"| ESP["ESP32<br/>(Motors, Sensors, Camera)"]
```
...
Before designing a use case, understand what your hardware can actually do. Refer to the [Cyberwave hardware documentation](/hardware) for detailed specs on each supported platform.
...
Go back to the Perceive -> Reason -> Act cycle from Section 2 and map each component to your specific hardware. This is where the architecture becomes concrete.

**Example with SO101:**

| Component | What It Is |
|---|---|
| **Perceive** | A USB camera attached to your laptop captures the workspace — objects, positions, arm state |
...
| **Act** | The tracked wheels drive the rover — forward, turn, stop, reverse |
...
Your robot doesn't operate in a vacuum. The environment shapes what's possible and what's hard.
...
- **Environmental factors** — Lighting conditions for camera-based perception, surface types for rover traction, obstacles that may appear or move.
...
1. **Target persona** — Who benefits from this? A warehouse worker? A lab technician? A home user? A farmer?
2. **Target environment** — Where does this run? A desk? A factory floor? An outdoor field? A kitchen?
3. **The task** — What specific physical task does the agent perform? Be concrete: "pick up a red block and place it in a bin" is better than "manipulation tasks."

Your use case doesn't need to be groundbreaking — it needs to be **testable, measurable, and completable within the program timeline**. A simple use case executed well is more valuable than an ambitious one that never works.

### Step 6: Design the System Loop

Take your use case and draw out the full system as a loop:

1. **Sensor** — What data does the agent need to perceive the task state? (camera frames, joint positions, distance readings)
2. **Model / Policy** — What decides the next action? Start with a rule-based approach, then graduate to a trained model
3. **Actuator** — What physically executes the action? (arm joints, wheel motors, gripper)
4. **Feedback** — How do you know if the action succeeded? (vision check, position comparison, force reading)
...
2. **Build a dataset** — Collect enough episodes (start with 20-50) covering variations of the task — different object positions, lighting conditions, starting states.
...
This is the core loop of building a physical AI agent: **demonstrate -> record -> train -> deploy -> test -> improve**.
...
Use these before or during the program to reduce ramp-up time.

### 6.1 Core Concepts

- Basics of embodied AI systems
- ROS 2 topics/services/actions and runtime introspection
- MQTT fundamentals for low-latency command/telemetry flows
- Dataset quality principles for robotics learning

### 6.2 Recommended Hands-On Prep

- Build one ROS pub/sub mini-project
- Send/receive structured MQTT messages
- Run one end-to-end sensor -> command -> actuator test
- Practice failure drills (network drop, process crash, restart)

### 6.3 Optional Research Read

For deeper conceptual grounding on physical AI agents and modular architecture patterns, read:

- [Physical AI Agents: Integrating Cognitive Intelligence with Real-World Action (arXiv)](https://arxiv.org/pdf/2501.08944v1)
