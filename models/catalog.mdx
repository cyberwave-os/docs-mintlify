...
Browse and load catalog rows from Python with **`cw.models`** — see [ML Models (Python SDK)](/sdks/models/overview). Typed **`create`** / **`update`** helpers are not on `cw.models` yet; call `POST` / `PUT` on this API or use generated `cw.api.*` wrappers until SDK methods land.
...
<CardGroup cols={2}>
  ...
  <Card title="ML Models (UI overview)" icon="brain" href="/use-cyberwave/ml-models/overview">
