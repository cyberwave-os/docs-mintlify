---
title: "Control an SO-101 arm with your voice"
sidebarTitle: "SO-101: Voice Control"
...
<iframe
  ...
  src="https://www.youtube.com/embed/kUxSxCMCQgc"
  title="SO-101 voice + vision agent demo"
  frameBorder="0"
...
<Info>
  **Reference implementation.** Full source for every module and smoke test
  referenced below lives in the
  [`nl_arm_controller`](https://github.com/cyberwave-os/cyberwave-python/tree/main/examples/nl_arm_controller)
  example in the
  [`cyberwave-os/cyberwave-python`](https://github.com/cyberwave-os/cyberwave-python)
  repo. Clone it to follow along, or use the snippets in each step as a guide
...
<Info>
  **Who provides what.** Cyberwave provides: digital twin, Python SDK, MQTT
  transport, edge driver, Live Mode. You provide: the laptop, the USB webcam,
  API keys for Anthropic Claude and Mistral Voxtral.
</Info>
...
- **Base setup**: [SO-101 Get Started](https://cyberwave.com/the-robot-studio/so101) for the environment, edge install, and `so101-remoteoperate` running on the Pi.
- **SDK baseline**: [Python SDK](/overview/tools/python-sdk).
- **Conceptual grounding**: [Key Concepts](/feature-reference/architecture/key-concepts), [Voice as a Sensor](https://cyberwave.com/catalog/tag/microphone).

---

## Project layout

The reference example lives at [`cyberwave-os/cyberwave-python/examples/nl_arm_controller`](https://github.com/cyberwave-os/cyberwave-python/tree/main/examples/nl_arm_controller):
...
Direct links: [`nl_arm_controller.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/nl_arm_controller.py) · [`motion.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/motion.py) · [`planner.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/planner.py) · [`vision.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/vision.py) · [`voice.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/voice.py) · [`requirements.txt`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/requirements.txt) · [`.env.example`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/.env.example) · [`smoke_tests/`](https://github.com/cyberwave-os/cyberwave-python/tree/main/examples/nl_arm_controller/smoke_tests)
...
Before you build the agent, you need a working SO-101 twin and a Cyberwave environment with the edge driver running. The full reference is in [SO-101 Get Started](https://cyberwave.com/the-robot-studio/so101). The short version:
...
2. Install Cyberwave Edge on the Raspberry Pi paired to the follower:
   ...
   ```
3. Pair the SO101 twin and calibrate the follower (dashboard or CLI). See [Calibrate the arms](https://cyberwave.com/the-robot-studio/so101#step-3-calibrate-the-arms).
...
<Check>
  ...
  `so101-remoteoperate` is running on the Pi (visible as an active driver under
  the twin).
</Check>
...
The agent needs to send joint commands from your laptop to the follower without a physical leader arm. That means assigning a controller to the twin in the dashboard and verifying the cloud-to-edge path with the keyboard _before_ you wire any LLM.
...
<Tip>
  **Why bother with the keyboard step?** Once the keyboard works, every later
  failure is in *your* code, not in calibration, pairing, MQTT, or the edge
  driver. It rules out four moving parts in two minutes.
</Tip>

For background on how remote operation differs from leader/follower teleoperation, see [SO-101 Get Started: Set Up Remote Operation](https://cyberwave.com/the-robot-studio/so101#set-up-remote-operation) and [Teleoperation](/feature-reference/environment-editor/teleoperation).

<Check>
  You're done with Step 2 when keyboard input in the dashboard moves the
  physical arm, and a one-liner Python script using
  `cw.twin(...).joints.set("1", 10, degrees=True)` also moves it. The platform
  automatically swaps the keyboard controller for the SDK session when your
  script connects, so you don't need to manually unassign anything.
</Check>
...
| ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------- | --------------------------------- | ------------------ |
| [`voice.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/voice.py)                         | Mic audio (while SPACE held) | Transcribed text                  | Mistral Voxtral    |
| [`vision.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/vision.py)                       | Webcam                       | Base64 JPEG                       | n/a (local OpenCV) |
| [`planner.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/planner.py)                     | Text + base64 JPEG           | Validated `MotionPlan`            | Anthropic Claude   |
| [`motion.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/motion.py)                       | `MotionPlan`                 | Ramped joint commands             | Cyberwave SDK      |
| [`nl_arm_controller.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/nl_arm_controller.py) | CLI flags                    | Agent loop wiring it all together | n/a                |
...
cd cyberwave-python/examples/nl_arm_controller
...
[`voice.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/voice.py) is a push-to-talk mic recorder plus a Mistral Voxtral STT call. Hold SPACE to record, release to transcribe. Output is plain text.
...
<Tip>
  **Voice does not reach Claude directly.** The path is: audio, then Voxtral,
  then text, then Claude. Claude is a text+image model, so the speech-to-text
  step is mandatory.
</Tip>
...
[`vision.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/vision.py) wraps `cv2.VideoCapture` and exposes `grab_frame_b64()`, which returns a base64-encoded JPEG ready to drop into a Claude `image` content block. No model runs locally; OpenCV is just for capture and JPEG encoding.
...
[`planner.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/planner.py) calls Anthropic Claude Sonnet 4.5 with a system prompt, the user transcript, and (optionally) a base64 JPEG. It returns a validated `MotionPlan` or a structured error.

The whole tutorial hinges on Claude returning a _strict, machine-parseable_ motion plan, not prose. We do this in three layers, before any code parses anything.
...
Example plan Claude returns for _"wave at the audience"_:
...
The vision-aware prompt (`VISION_SYSTEM_PROMPT`) adds **decision rules**: "describe only: empty actions", "motion only: narrate briefly", "visually-grounded motion: describe + aim with joint 1", plus a rule for honest _"no, I don't see that"_ answers with empty actions.
...
4. **Per-joint clamping**: every commanded angle is squeezed into `DEFAULT_JOINT_LIMITS` _right before_ the SDK call. If Claude emits `angle: 9999`, the arm receives `90`. The arm is physically incapable of executing an out-of-range request.
...
<Warning>
  Joint limits in `motion.DEFAULT_JOINT_LIMITS` are *conservative*: ±90° on the
  base, ±60° on every other joint. Wide enough to look expressive on a
  public-demo arm; narrow enough that worst-case the arm can't bash itself, the
  table, or you. Loosen at your own risk.
</Warning>

### 3.4 Orchestrate it together

[`motion.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/motion.py) and [`nl_arm_controller.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/nl_arm_controller.py) close the loop. The executor turns plans into smooth motion; the orchestrator owns the agent loop.
...
**The agent loop** (in [`nl_arm_controller.py`](https://github.com/cyberwave-os/cyberwave-python/blob/main/examples/nl_arm_controller/nl_arm_controller.py)):
...
_Expansion point: trapezoidal or S-curve velocity profiles for even smoother starts and stops. Not wired in yet; linear at 20 Hz is plenty for a demo._
...
<Steps>
  ...
  <Step title="Aim the webcam">
    Point the USB webcam at the SO-101 workspace. Use the JPEG saved by smoke
    test 07 to confirm framing and lighting.
  </Step>
  ...
  <Step title="Speak a command">
    Hold SPACE, say *"wave at the audience"*, release. Watch the terminal print
    the transcript, the planner latency, and the JSON plan, then watch the arm
    execute it.
  </Step>
  <Step title="Try vision-grounded prompts">
    *"what do you see in front of you?"*: expect a description, **no motion**.
    *"is there a red cup?"*: expect a yes/no grounded in the actual frame.
    *"look at the red cup"*: expect a small base rotation toward whatever Claude
    identifies.
  </Step>
  <Step title="Exit cleanly">
    Say or type `bye`, or press `Ctrl+C`. The `finally` block always homes the
    arm before disconnecting.
  </Step>
</Steps>
...
- **On-robot microphone (Pattern B)**: mic and STT on the edge, transcript recorded as telemetry. See [Voice as a Sensor](https://cyberwave.com/catalog/tag/microphone).
...
- **Example folder**: [`cyberwave-os/cyberwave-python/examples/nl_arm_controller`](https://github.com/cyberwave-os/cyberwave-python/tree/main/examples/nl_arm_controller) (smoke tests under [`smoke_tests/`](https://github.com/cyberwave-os/cyberwave-python/tree/main/examples/nl_arm_controller/smoke_tests))
- **SDK calls used**: `Cyberwave()`, `cw.affect()`, `cw.twin()`, `robot.joints.set()`. See [Python SDK](/overview/tools/python-sdk).
...
- **Cross-links**: [SO-101 Get Started](https://cyberwave.com/the-robot-studio/so101), [SO-101 Voice Pick-and-Place](/tutorials/so101-voice-pick-and-place), [Voice as a Sensor](https://cyberwave.com/catalog/tag/microphone), [Key Concepts](/feature-reference/architecture/key-concepts), [Teleoperation](/feature-reference/environment-editor/teleoperation).
