...
The SDK uses a shared command interface for simulation and hardware, but changing
the mode is not sufficient to validate a policy for a real robot. First verify
the observation/action contract, calibration, timing, limits and stop behavior,
then obtain explicit approval for a bounded physical trial.
...
If your policy reads the **wrist camera**, the real twin can supply camera frames,
but transfer is not automatic. Validate camera calibration, image preprocessing,
viewpoint, latency and robustness to real lighting and appearances before any
physical trial:
