---
title: "Fold cloth with two SO-101 arms"
sidebarTitle: "Cloth Folding (Dual SO-101)"
...
- **Base setup**: complete [SO-101 Get Started](https://cyberwave.com/the-robot-studio/so101) for one arm pair before scaling to two. The teleop and calibration steps generalize directly.
...
You need one Cyberwave environment that contains **both** SO-101 arm pairs and the RealSense camera, all paired to the hardware on your edge device. The full reference is in [SO-101 Get Started: Set Up the Cyberwave Environment](https://cyberwave.com/the-robot-studio/so101#step-1-set-up-the-cyberwave-environment).

1. In the Cyberwave dashboard, click **New Environment** and give it a name (e.g. _"Cloth Folding"_).
...
Calibration teaches the software where each joint's zero position is, what its valid movement range is, and how each physical arm maps to its software model. Without it, joint commands won't translate correctly to hardware. For the full reference, see [SO-101 Get Started](https://cyberwave.com/the-robot-studio/so101#step-3-calibrate-the-arms).
...
- The natural-language task prompt (e.g. _"fold a napkin"_).
...
<CardGroup cols={3}>
  <Card
    title="Project blog"
    icon="book-open"
    href="https://cyberwave-bostonx-cloth-folding-so101.netlify.app"
  >
  ...
  <Card
    ...
    icon="github"
    href="https://github.com/apavani2/cyberwave-cloth-folding-so101"
  >
    Clone the dual-arm pipeline: data collection, conversion, training,
    inference, and the Rerun visualizer.
  </Card>
  <Card
    title="Sandwich-making with SmolVLA"
    icon="burger"
    href="/tutorials/sandwich-robot-smolvla"
  >
...
_Built by Abhishek Pavani and Yash Shukla (Team BostonX) as part of the Cyberwave Builder Program._
