...
- A twin bound to a camera with the
  [generic camera driver](/edge/drivers/generic-camera) and
...
2. **Add the trigger**: drag a `Camera Frame` node from the
   **Sources & Triggers** group, set `twin_uuid` to the camera twin.
...
7. **Configure the alert** on the `Call Model` node's `emit_event`
   parameters: set `event_type` (e.g. `intrusion`), `severity`
   (`warning`), and leave `enabled=true`. The edge worker emits the
   alert directly on the dwell-fired frame using these settings —
   wiring an explicit `Send Alert` node downstream of
   `timed_condition` in the same `camera_frame` workflow is harmless
   but ignored by the edge codegen (the edge worker bypasses the
   cloud runner for per-frame triggers).
