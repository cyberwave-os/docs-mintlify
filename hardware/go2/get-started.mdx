...
<Info>
You don't have to hardcode the Go2's IP. The Cyberwave ROS 2 driver
automatically probes `192.168.12.1` and `192.168.123.161` on startup and uses
the first address that responds. If your Go2 is on a custom subnet, set
`ROBOT_IP` or `ROBOT_IP_FALLBACKS` in the environment. See [Robot IP
discovery](/hardware/go2/ros2-driver#robot-ip-discovery) for details.
</Info>
