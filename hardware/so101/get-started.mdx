...
<Accordion title="macOS: Camera stream fallback (if USB/IP video is slow)">

If USB/IP video bandwidth is insufficient for your camera resolution, you can use an MJPEG stream fallback instead. This captures from the macOS camera via `ffmpeg` and serves it as an HTTP stream.
...
```bash
# Start ffmpeg MJPEG server on macOS host:
ffmpeg -f avfoundation -framerate 30 -video_size 640x480 -i "0" \
  -c:v mjpeg -q:v 5 -f mjpeg -listen 1 http://0.0.0.0:8091

# Tell edge-core to use the stream URL instead of /dev/video*:
export CYBERWAVE_MACOS_CAMERA_STREAM_URL=http://host.docker.internal:8091
```

The driver's `cv2.VideoCapture` natively supports HTTP MJPEG URLs — no driver code changes needed. Note: this fallback does not work for Intel RealSense depth cameras (which require raw USB access).
