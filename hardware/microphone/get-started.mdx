...
<Tabs>
  ...
  <Tab title="Pattern B: On-robot microphone">
    ...
    ```
    Edge mic -> native microphone driver -> WebRTC audio stream (48 kHz Opus, resampled)
                                      ├─> Zenoh audio/default (raw PCM @ hardware rate, with metadata)
                                      └─> MQTT start_audio / stop_audio control
    ```

    The native driver uses `sounddevice` / PortAudio for host capture, streams audio with the Python SDK's `MicrophoneAudioStreamer`, publishes raw chunks to Zenoh for `on_audio` worker hooks, and sends `start_recording` / `stop_recording` commands to the media service without restarting WebRTC. See the [native driver setup guide](/edge/drivers/native-microphone-driver).
  </Tab>
</Tabs>

<Info>
For the driver source and local run examples, see the [Generic Microphone Driver README](https://github.com/cyberwave-os/generic-microphone-driver).
</Info>
