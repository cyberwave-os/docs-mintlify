...
<Note>
  The Raspberry Pi host is accessible on **port 22** (for system and network
  tasks), while the Docker container is accessible on **port 23** (for ROS 2 and
  Cyberwave configuration). These are two separate SSH sessions into different
...
<Warning>
  **Copy the Twin UUID** — you'll need it in the next step. Hover over the three
  dots next to the asset in the sidebar and click **Copy Twin UUID**.
</Warning>
...
<Check>
  **Connection Established:** If successful, you should see the digital twin in
  your Cyberwave environment updating in real time with the robot's joint states
  and sensor data.
</Check>
